import { retirementLedger } from "@/lib/mockData";

export default function RetirementLedgerTable() {
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        <h2 className="text-lg font-semibold text-gray-900">Retirement Fund Ledger</h2>
      </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {retirementLedger.map((entry, index) => (
              <tr key={index} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-sm font-semibold text-blue-600 text-right">AED {entry.contribution.toLocaleString()}</td>
      ...
      <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
        <div className="grid grid-cols-2 gap-4">
          <p className="text-sm text-gray-600">
            ...
            {(retirementLedger[0].runningBalance - retirementLedger[retirementLedger.length - 1].runningBalance + retirementLedger[retirementLedger.length - 1].contribution).toLocaleString()}
          </p>
          <p className="text-sm text-gray-600 text-right">
            <span className="font-semibold text-gray-900">Current Balance:</span> AED{" "}
            {retirementLedger[0].runningBalance.toLocaleString()}
          </p>
        </div>
      </div>
    </div>
